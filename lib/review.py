...
class Review:
    ...
    def year(self, new_year):
        if isinstance(new_year, int) and new_year >= 2000:
            ...
            raise Exception('Year must be after 2000!')
    ...
    def summary(self, new_summary):
        if new_summary == '':
            raise Exception('Cant be an empty string!')
        else:
    ...
    def instance_from_db(cls, row):
        ...
        if review:
            # ensure attributes match row values in case local instance was modifed
            review.year = row[1]
